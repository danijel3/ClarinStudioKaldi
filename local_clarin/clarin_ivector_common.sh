...
if [ $stage -le 1 ]; then
  ...
  #the subset is ~10% of the train corpus
  utils/subset_data_dir.sh --first data/train 1254 data/train_ss || exit 1
  utils/subset_data_dir.sh --first data/train_hires 1254 data/train_ss_hires || exit 1
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
     --num-frames 400000 data/train_ss_hires 256 exp/nnet3/tri5b exp/nnet3/diag_ubm
fi
...
if [ $stage -le 7 ]; then
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 8 \
      data/test_hires exp/nnet3/extractor exp/nnet3/ivectors_test || echo "$0: error extracting iVectors." && exit 1;
fi
